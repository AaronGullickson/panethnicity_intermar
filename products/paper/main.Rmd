...
* Qian, Blair, and Ruf (NEED TO READ). 1990 Census data. 
* Rosenfeld 2001. (NEED TO READ) The most cited and a *Demography* paper. Need to re-read to get a better understanding of methods and results. 
* Qian and Cobas, 2004. (NEED TO READ) Only looks at Latinos, uses 1990 Census data. Uses the race of Latinos as a variable, something he and others do in later papers - not a big fan of that approach.
...
* Okamoto, 2007. (NEED TO READ) Looks specifically at Asian pan-ethnic intermarriage. 
* Qian, Glick, and Batson, 2012. (NEED TO READ) Uses Census 2000 data to look at four groups of Chinese, Filipinos, Mexicans, and Puerto Ricans.
...
groups <- c("Chinese","Korean","Japanese","Vietnamese","Filipino")
or <- coefs_extended %>%
...
groups <- c("Mexican","Puerto Rican","Cuban","Guatemalan",
...
gg.blacklatino <- coefs_extended %>%
  ...
  scale_x_continuous(limits=c(0,1))+
  labs(y=NULL, 
       x="Odds ratio of black/Latino intermarriage\nrelative to ethnoracial endogamy")+
  theme_mine()+
  theme(axis.title = element_text(size=8))

groups <- c("Mexican","Puerto Rican","Cuban")
gg.blackchange <- coefs_extended %>%
  filter(grepl(paste(groups,collapse="|"), variable) & 
           grepl("Black", variable) & 
           model=="+ both" & 
           data!="ACS 2014-18, expanded ethnic groups") %>%
  ggplot(aes(x=variable, y=exp(coef)))+  
  geom_hline(yintercept = 1, color="grey20", linetype=3)+
  ...
  scale_color_manual(values=my_palette)+
  labs(x=NULL, 
       y="Odds ratio of black/Latino intermarriage\nrelative to ethnoracial endogamy in ACS data",
       color="Data source")+
  theme_mine()+
...
grid.newpage()
print(gg.blackchange, vp = viewport(x = 0.25, y = 0.5, width = 0.5, height = 1.0))
print(gg.blacklatino, vp = viewport(x = 0.75, y = 0.5, width = 0.5, height = 1.0))
```
