...
thanks: A replication package containing code and data for this article is available at [https://doi.org/10.17605/OSF.IO/HZJ4Q](https://doi.org/10.17605/OSF.IO/HZJ4Q) or on [GitHub](https://github.com/AaronGullickson/panethnicity_intermar).
  ...
  mutate(variable=convert_intermar_names(variable, "race_exog_pent"),
         ...
         data=factor(data,
                     ...
                     labels=c("Census 1980", 
                              "ACS 2014-2018, 1980 ethnic groups",
                              "ACS 2014-2018, all ethnic groups"))) %>%
...
coefs_extended <- list(census=models_census_extended,
  ...
  mutate(variable=convert_intermar_names(variable, "race_exog_extended"),
         ...
         data=factor(data,
                     ...
                     labels=c("Census 1980", 
                              "ACS 2014–2018, 1980 ethnic groups",
                              "ACS 2014-2018, expanded ethnic groups"))) %>%
  order_variables("Census 1980", "+ both")
...
temp2 <- temp %>%
  ...
  summarize(diversity_bpl=1-sum(p^2)) %>% 
  ungroup() %>%
  mutate(data="ACS 2014-2018, all ethnic groups")
...
temp2 <- temp %>%
  ...
  summarize(diversity_bpl=1-sum(p^2)) %>% 
  ungroup() %>%
  mutate(data="ACS 2014-2018, 1980 ethnic groups")
...
temp2 <- diversity_ethnic %>%
  ...
  ungroup() %>%
  mutate(data=ifelse(data=="ACS 2014-18", "ACS 2014-2018, all ethnic groups",
                     data))
...
diversity_pan %>%
  ...
  labs(y=NULL, x="Probability of Not Sharing the Same Birthplace/Language",
       color=NULL)+
  theme_mine()+
...
coefs_pent %>%
  ...
  mutate(variable=str_replace(variable, "/", "-"),
         ...
         coef=ifelse(data=="ACS 2014–2018, all ethnic groups" & variable=="White/Black", NA, coef)) %>%
...
or_asian <- coefs_extended %>%
  filter(data=="ACS 2014-2018, expanded ethnic groups" &
           model=="+ both" &
...
gg.heat.asian <- ggplot(melt(1/or_asian, na.rm=TRUE), aes(Var2, Var1, fill = value))+
  ...
  scale_fill_gradient2(low = "#7b3294", high = "#008837", mid = "#f7f7f7", 
                       ...
                       name="Odds ratio")+
  geom_text(aes(label=c(format(round(1/or_asian,2), nsmall = 2))))+
  labs(x=NULL, y=NULL)+
  theme_minimal()+
...
gg.heat.latino <- ggplot(melt(1/or_latino, na.rm=TRUE), aes(Var2, Var1, fill = value))+
  ...
  scale_fill_gradient2(low = "#7b3294", high = "#008837", mid = "#f7f7f7", 
                       midpoint = 1, limit = c(0, 1.25),
                       name="Odds ratio")+
  geom_text(aes(label=c(format(round(1/or_latino,2), nsmall = 2))), size=2.5)+
  labs(x=NULL, y=NULL)+
  theme_minimal()+
  theme(legend.position = "left", 
        panel.grid.major = element_blank(),
...
panethnic <- coefs_extended %>%
  filter(grepl(paste(c(latino_groups,asian_groups),collapse="|"), variable) & 
           ...
           model=="+ both" & 
           data=="ACS 2014-2018, expanded ethnic groups") %>%
