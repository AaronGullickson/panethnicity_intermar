...
To address the potential for this method to miss important heterogeneity in the likelihood of ethnoracial exogamy, I consider an additional less parsimonious model using the ACS data. In this model, I code each specific combination of ethnic categories within the same panethnic category using a separate dummy variable. Additionally, I treat each combination of an ethnic category among the White and Black categories as separate variable which allows me to also determine whether the tendency to outmarry with Whites or Blacks varies among ethnic groups within the same panethnic category. I restrict the ethnic categories in this model, to those for which I can fit a model without problems of sparseness and model non-convergence. I determine the most expansive model that can be fit by experimentation. This model includes the five East/Southeast Asian ethnic groups available in the Census 1980 data (Chinese, Filipino, Vietnamese, Korean, and Japanese) and the ten largest Latino groups, with the exception of Hondurans. I could not fit ethnicity specific models for South Asians because of the small size of non-Asian Indian groups in the South Asian category. These models allow me to explore in greater detail heterogeneity between specific ethnic groups within a given panethnic category at a significant loss to parsimony. The model estimates 85 separate exogamy terms (==CHECK==). 

==DISCUSS HOW TO INTERPRET TERMS==
