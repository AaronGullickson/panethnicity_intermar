...
```{r asian-heat-map, fig.cap="A Caption"}
...
gg.heat <- ggplot(melt(or, na.rm=TRUE), aes(Var2, Var1, fill = value))+
  geom_tile(color = NA)+
  scale_fill_gradient2(low = "#008837", high = "#7b3294", mid = "#f7f7f7", 
...
```{r latino-heat-map, fig.cap="A Caption"}
...
gg.heat <- ggplot(melt(or, na.rm=TRUE), aes(Var2, Var1, fill = value))+
  ...
  scale_fill_gradient2(low = "#008837", high = "#7b3294", mid = "#f7f7f7", 
                       midpoint = 1, limit = c(0, max(or)),
                       name="odds ratio")+
  geom_text(aes(label=paste(round(or,2))), size=2.5)+
  labs(x=NULL, y=NULL)+
  theme_minimal()+
  theme(legend.position = "left", 
        panel.grid.major = element_blank(),
...
```{r black-exogamy, fig.cap="A Caption"}
groups <- c("Mexican","Puerto Rican","Cuban","Guatemalan",
            "Salvadorian","Colombian","Ecuadorian","Peruvian",
            "Dominican")
