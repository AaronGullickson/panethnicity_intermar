...
the results here are quite similar to the E&SE Asian case. The baseline ethnic exogamy is even higher and the increases with birthplace and language endogamy controls put ethnic exogamy at almost the exact same value as for E&SE Asians. It suggests to "ethnic" melting pots for Asians. 
...
```{r deviance-bendog}
sapply(models_bendog, function(x) {-2*x$loglik[2]}) %>%
  enframe(name="model",value="deviance") %>%
  mutate(gen1.75=c("Birthplace","USA","Both",rep("USA",5),rep("Both",2)),
         gen1.5 =c("Birthplace","USA","Both","Birthplace",rep("Both",2),
                   rep("USA",2),"Birthplace","Both"),
         gen1.25=c("Birthplace","USA","Both",rep("Birthplace",2),rep("Both",2),
                   rep("Birthplace",3))) %>%
