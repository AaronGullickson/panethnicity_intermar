...
df_arrows <- coef_table %>%
  ...
  mutate(coef.census=ifelse(abs(exp(coef.census)-exp(coef.acs))<0.05, NA,
                            coef.census))
...
coef_table %>%
  filter(model=="+ both" & racial_exog & grepl("South Asian",variable)) %>%
  select(variable, coef, data) %>%
  ggplot(aes(x=variable, y=exp(coef)))+
  geom_hline(yintercept = 1, linetype=2)+
  geom_point(aes(color=data), size=2, alpha=0.8)+
  geom_segment(data=subset(df_arrows, grepl("South Asian",variable)), 
               aes(x=variable, xend=variable, 
                   y=exp(coef.census), yend=exp(coef.acs)),
               arrow = arrow(length = unit(0.02, "npc")),
               color="grey60")+
