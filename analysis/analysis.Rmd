...
heatmap_ethnic <- function(groups, model_name, y=0.6, height=0.68) {
  ...
  or <- 1/or[hc$order, rev(hc$order)]
  
  gg.heat <- ggplot(melt(or, na.rm=TRUE), aes(Var2, Var1, fill = value))+
    geom_tile(color = NA)+
    scale_fill_gradient2(high = "#276419", low = "#8e0152", mid = "#f7f7f7", 
...
coefs_extended %>%
  ...
  select(ethnicity, race, coef) %>%
  ggplot(aes(y=reorder(ethnicity, coef, max), x=exp(coef), color=race))+
  geom_vline(xintercept = 0, color="grey70", linetype=1)+
  geom_vline(xintercept = 1, color="grey70", linetype=3)+
  geom_pointrangeh(aes(xmin=0, xmax=exp(coef)),
                   position=position_dodgev(height=0.5))+
  ...
  scale_x_continuous(limits=c(0,1))+
  labs(y=NULL, 
       x="Odds ratio of union formation for Latino interracial marriage relative to ethnic endogamy",
       color="Interracial partner",
       title="Odds ratio of Latino interracial marriage to whites and blacks",
       subtitle="ACS Data")+
  theme_bw()+
  theme(legend.position = "right")
```
...
ggplot(intermar, 
  ...
  labs(y=NULL, 
       ...
       color="Interracial partner",
       title="Odds ratio of interracial marriage",
       subtitle="ethnic exogamy terms shown in grey",
       caption="ACS Data, 2014-2018")+
  theme_bw()+
...
coefs_extended %>%
  ...
  scale_x_continuous(limits=c(0,1))+
  labs(y=NULL, 
       x="Odds ratio of union formation for Asian interracial marriage relative to ethnic endogamy",
       color="Interracial partner",
       title="Odds ratio of Asian interracial marriage to whites and blacks",
       subtitle="ACS Data")+
  theme_bw()+
  theme(legend.position = "right")
```
...
sapply(models_bendog_census, function(x) {mean(x$deviance)}) %>%
  ...
  relocate(model, gen1.75, gen1.5, gen1.25, deviance_census, deviance_acs) %>%
