...
alternates_acs %>%
  ...
  mutate(lang=factor(case_when(
    ...
    lang==5300 ~ "Malay",
    lang==1100 ~ "French",
    lang>=4000 & lang<5500 ~ "Other Asian Language",
    TRUE ~ "Other Language"),
  ...
  scale_fill_manual(values = c("grey", brewer.pal(7, "Dark2"),
                               ...
                               brewer.pal(7,"Set1"),"paleturquoise1","grey30"))+
...
alternates_acs %>%
  ...
  mutate(lang=factor(case_when(
    ...
    lang==4004 ~ "Malayalam",
    lang==3114 ~ "Marathi",
    lang==3010 ~ "Pashto, Afghan",
    TRUE ~ "Other Language"),
    levels=c("English","Hindi/Urdu","Gujarathi","Panjabi","Bengali",
             "Marathi","Sinhalese","Telugu","Tamil","Malayalam",
             "Pashto, Afghan","Other Language")),
    race=fct_reorder(race, lang=="English", mean)
  ) %>%
  select(race, lang) %>%
  droplevels() %>%
  ggplot(aes(x=race, fill=fct_rev(lang)))+
  geom_bar(position="fill")+
  scale_fill_manual(values = c("grey",brewer.pal(4, "Dark2"),
                               brewer.pal(6, "Pastel1"),"grey30"))+
  scale_y_continuous(labels = scales::percent)+
  labs(x=NULL, y="cumulative percent",
...
alternates_acs %>%
  ...
  mutate(birthplace=factor(case_when(
    ...
    levels=c("USA","China","Hong Kong","Macau","Taiwan","Japan","Korea",
             ...
             "Singapore","Thailand","Vietnam","Myanmar","Other Place")),
  ...
  scale_fill_manual(values = c("grey", brewer.pal(7, "Dark2"),
                               "tomato","sienna","orchid","royalblue","maroon",
                               "paleturquoise1","olivedrab", "grey30"))+
  scale_y_continuous(labels = scales::percent)+
  labs(x=NULL, y="cumulative percent",
...
diversity_pan <- temp %>%
  ...
  full_join(temp2) %>%
  bind_rows(diversity_pan)  %>%
  mutate(type="within pan-ethnic group")

# ggplot(diversity_ethnic, aes(x=diversity_lang, y=diversity_bpl,
#                              color=group))+
#   geom_point()+
#   #geom_point(data=diversity_pan, shape=17, size=5)+
#   facet_wrap(~data)+
#   theme_bw()
...
ggplot(diversity_pan, aes(x=group, y=diversity, fill=type))+
  ...
  scale_y_continuous(labels = scales::percent)+
  labs(x=NULL, y="Probability two members do not share the same category")+
  #coord_flip()+
  theme_bw()+
...
```{r diversity-full, fig.height=10}
temp <- diversity_pan %>%
  filter((data=="ACS 2014-18\nall ethnic groups" | data=="Census 1980")) %>%
  mutate(data=ifelse(data=="ACS 2014-18\nall ethnic groups",
                     "ACS 2014-18", as.character(data)))
...
dg <- diversity_ethnic %>%
  ...
  mutate(variable=ifelse(variable=="diversity_bpl",
                         "Birthplace diversity",
                         "Language diversity"),
         data=relevel(factor(data), "Census 1980")) %>%
  arrange(data, variable, diversity) %>%
  mutate(order = row_number())

ggplot(dg, aes(y=order, x=diversity, color=group))+
  geom_pointrange(aes(xmin=0, xmax=diversity),
                  position=position_dodgev(height=1), 
                  alpha=0.8)+
  geom_vline(data=temp, 
             aes(xintercept=diversity, color=group, 
                 linetype=type))+
  scale_x_continuous(labels=scales::percent, expand=c(0,0),
                     limits = c(0,0.85))+
  #scale_color_manual(values=my_palette)+
  scale_linetype_manual(values=c(4,1))+
  scale_y_continuous(breaks = dg$order, labels = dg$race,
                     expand = c(0,0))+
  facet_wrap(variable~relevel(factor(data), "Census 1980"),
             scales="free_y")+
  labs(x="Diversity (Gini-Simpson Index)",
       y=NULL,
       caption="vertical lines give pan-ethnic group diversity",
       color="Pan-ethnic group")+
  theme_bw()+
...
temp <- alternates_acs %>%
  filter(race!="AIAN" & race!="White" & race!="Black") %>%
  group_by(race) %>%
  summarize(prop_english=mean(lang==100, na.rm=TRUE),
            prop_born_us=mean(bpld==1, na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(data="ACS 2014-18")
...
temp <- alternates_census %>%
  ...
  bind_rows(temp) %>%
  full_join(diversity_ethnic) %>%
  mutate(data=relevel(factor(data), "Census 1980"))

theoretical <- tibble(x=seq(from=0.01,to=0.99, by=0.01),
                      y=1-(x^2+(1-x)^2))

ggplot(temp, aes(x=prop_english, y=diversity_lang, color=group))+
  geom_point()+
  geom_smooth(se=FALSE, method="lm", formula=y~x+I(x^2))+
  geom_line(data=theoretical, aes(x=x, y=y), color="grey",
            linetype=2)+
  theme_bw()+
  scale_y_continuous(labels=scales::percent)+
  scale_x_continuous(labels=scales::percent)+
  labs(x="Percent who speak English as primary language",
       y="Language diversity (Gini-Simpson Index)",
       color="Pan-ethnic group",
       caption="dashed grey line is exact diversity index in two group situation")+
  facet_wrap(~data)

ggplot(temp, aes(x=prop_born_us, y=diversity_bpl, color=group))+
  geom_point()+
  geom_smooth(se=FALSE, method="lm", formula=y~x+I(x^2))+
  geom_line(data=theoretical, aes(x=x, y=y), color="grey",
            linetype=2)+
  ...
  scale_y_continuous(labels=scales::percent)+
  scale_x_continuous(labels=scales::percent)+
  labs(x="Percent who were born in the US",
       y="Birthplace diversity (Gini-Simpson Index)",
       color="Pan-ethnic group",
       caption="dashed grey line is exact diversity index in two group situation")+
  facet_wrap(~data)
```
...
alternates_acs %>%
  filter(race %in% c("Mexican","Puerto Rican","Cuban","Costa Rican",
                     ...
                     "Colombian","Ecuadorian","Paraguayan","Peruvian",
                     "Uruguayan","Venezuelan","Dominican")) %>% 
  mutate(race_report=factor(case_when(
    raced==100 ~ "White",
    raced==200 ~ "Black",
    raced>=300 & raced<400 ~ "AIAN",
    raced>=400 & raced<700 ~"API",
    raced==700 ~ "Other",
    raced>=800 ~ "Multiracial"),
    levels=c("Multiracial","AIAN","API",
             "Other","Black","White")),
    race=fct_reorder(race, race_report=="White", mean)) %>%
  ggplot(aes(x=race, fill=race_report))+
  geom_bar(position="fill")+
  scale_fill_manual(values = c(brewer.pal(4, "Dark2"), "grey30", "grey90"))+
  scale_y_continuous(labels = scales::percent)+
  labs(x=NULL, y="cumulative percent",
       title="Race reporting among Latino ethnic groups",
       caption="Data Source: ACS 2014-18, partner alternates",
       fill="Reported race")+
  coord_flip()+
  theme_bw()
...
* **USA**: replace the country of birth with the USA so that they will be endogamous with someone born in the US and exogamous with someone born in their country of birth. Most assimilative choice.
* **Both**: Treat the person as endogamous with both someone born in the US and someone born in their country of birth. Intermediate assimilative choice.
* **Birthplace**: Treat their country of birth as their birthplace so that they will be exogamous with someone born in the US and endogamous with someone born in their country of birth. Least assimilative choice.
...
I created dummy variables for birthplace endogamy based on these 10 options and then ran a model on the ACS data with each dummy variable. I used the ACS data with the fullest set of ethnic groups, racial pentagon coding, and controls for age, educational assortative mating, and language endogamy. The results below show the deviance for each of the ten models, ranked from lowest deviance to highest deviance.

```{r deviance-bendog}
sapply(models_bendog, function(x) {mean(x$deviance)}) %>%
  enframe(name="model",value="deviance") %>%
  mutate(gen1.75=c("Birthplace","USA","Both","USA",       "USA",       "USA",       "USA", "USA", "Both",      "Both"),
         gen1.5 =c("Birthplace","USA","Both","Both",      "Birthplace","USA",       "Both","USA", "Birthplace","Both"),
         gen1.25=c("Birthplace","USA","Both","Birthplace","Birthplace","Birthplace","Both","Both","Birthplace","Birthplace")) %>%
  relocate(model, gen1.75, gen1.5, gen1.25, deviance) %>%
  arrange(deviance) %>%
  kbl(caption = "Deviance from models using different specifications of birthplace for 1.25, 1.5, and 1.75 generation") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F)
```

The least preferred options were cases that coded "USA" for most groups. The models clearly preferred "Birthplace" coding for the 1.25 generation, either "Both" or "Birthplace" coding for the 1.5 generation, and "Both" or "USA" coding for the 1.75 generation. The most preferred option is the "flex2" coding which codes "Both" for the 1.75 and 1.5 generation and "Birthplace" for the 1.75 generation. This is the coding that I use in all of the models reported in the analysis. 
...
```{r model-census-extended, results="asis"}
...
```{r model-acs1980-extended, results="asis"}
